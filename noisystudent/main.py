...
import os
os.environ['TF_USE_LEGACY_KERAS'] = '1'  # Ép dùng Keras bản cũ
os.environ['TF_ENABLE_ONEDNN_OPTS'] = '0' # Tắt cảnh báo floating-point
import json
import time
import math
import functools
...
import tensorflow as tf
# Đoạn code giả lập môi trường cũ
...
tf.gfile = tf.io.gfile
tf.train.get_global_step = tf.compat.v1.train.get_or_create_global_step
if not hasattr(tf.compat.v1.layers, 'DepthwiseConv2D'):
    tf.compat.v1.layers.DepthwiseConv2D = tf.keras.layers.DepthwiseConv2D
...
def model_fn(features, mode, params):
  ...
  batch_size = params.get('batch_size', 1)   # pylint: disable=unused-variable
  ...
  def build_model(scope=None, reuse=tf.compat.v1.AUTO_REUSE, model_name=None,
  ...
  lab_acc = tf.cast(tf.equal(lab_pred, lab_labels), dtype=tf.float32)
  ...
  # compute lab_loss
  one_hot_labels = tf.one_hot(lab_labels, FLAGS.num_label_classes)
  lab_loss = tf.compat.v1.losses.softmax_cross_entropy(
      logits=lab_logits,
  ...
  real_lab_bsz = tf.cast(lab_bsz, dtype=tf.float32) * FLAGS.label_data_sample_prob
  ...
  loss = data_loss + FLAGS.weight_decay * tf.add_n(
    [tf.cast(tf.nn.l2_loss(v), tf.float32) # Ép kiểu về float32 ở đây
  ...
  # 1. Đảm bảo logits là Rank 2 (Batch_size, Num_classes)
  # Nếu logits đang là [Batch], ta cần kiểm tra lại đầu ra của model_fn
      logits = tf.cast(logits, tf.float32)

      # 2. Xử lý Labels: in_top_k yêu cầu labels là Rank 1 [Batch] 
      # Nếu labels của Thành đang là One-hot [8, 2], ta phải lấy chỉ số có giá trị lớn nhất
      ...
      # Nếu logits vẫn báo lỗi Rank 1, ta ép nó về Rank 2 (giả định logits là xác suất lớp 1)
      if logits.shape.ndims == 1:
          # Biến logits từ [8] thành [8, 1] hoặc xử lý tùy theo logic model của Thành
          # Tuy nhiên thường logits từ EfficientNet phải là [Batch, 2]
          # Nếu bị lỗi này, có khả năng Thành đã dùng tf.argmax trước khi truyền vào đây.
          return {
              'precision_top_1': tf.compat.v1.metrics.accuracy(labels, tf.cast(logits, tf.int32))
          }

      in_top_1 = tf.cast(tf.nn.in_top_k(predictions=logits, targets=labels, k=1), tf.float32)
      
      return {
          'precision_top_1': tf.compat.v1.metrics.mean(in_top_1),
      }
...
def main(unused_argv):
  ...
  if FLAGS.mode == 'train' or FLAGS.mode == 'train_and_eval':
    ...
    flags_dict = tf.compat.v1.app.flags.FLAGS.flag_values_dict()
 ...
 # 1. Khởi tạo tpu_config ra ngoài hoàn toàn khối IF/ELSE
  ...
  est = tf.compat.v1.estimator.tpu.TPUEstimator(
  ...
  if FLAGS.mode == 'train' or FLAGS.mode == 'train_and_eval':
    ...
    if FLAGS.mode == 'train':
      ...
      # Ép chạy thêm 20000 bước tính từ bước hiện tại
      max_steps=FLAGS.train_steps,
      hooks=[])
  elif FLAGS.mode == 'eval':
    input_fn_mapping = {}
    eval_subsets = ['dev']
    if FLAGS.num_test_images:
      eval_subsets.append('test')
    for subset in eval_subsets:
  ...
  elif FLAGS.mode == 'export_only':
      tf.logging.info('--- Đang bắt đầu quá trình Export SavedModel ---')
      
      def image_serving_input_receiver_fn():
          # Tạo placeholder cho ảnh
          placeholder = tf.compat.v1.placeholder(
              dtype=tf.float32, 
              shape=[None, input_image_size, input_image_size, 3], 
              name='input_tensor'
          )
          
          # Cung cấp cả 'image' và 'images' để tránh lỗi KeyError
          receiver_tensors = {
              'image': placeholder,
              'images': placeholder
          }
          
          # Trả về đối tượng nhận dữ liệu
      ...
      est.export_saved_model(
          ...
          serving_input_receiver_fn=image_serving_input_receiver_fn
      )
      tf.logging.info('--- Export thành công! Kiểm tra thư mục model_output ---')
  else:
      print(f"Mode {FLAGS.mode} không được hỗ trợ hoặc chưa cấu hình logic.")
